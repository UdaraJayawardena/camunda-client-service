...
const deleteList = async (formData, auth) => {

  const { tenantIdIn } = formData;

  if (process.env.NODE_ENV === 'development') {

    const results = { errLog: [], ok: 0 };

    let getListQuery = { tenantIdIn: tenantIdIn };

    if (tenantIdIn === 'LI') getListQuery = {
      withoutTenantId: true
    };

    const flowList = await Service.getList(getListQuery, auth);
